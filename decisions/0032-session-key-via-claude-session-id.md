...
**Status:** Superseded by ADR-0033 (session-key source only; encryption envelope, persistence behavior, re-login flow, and hook coverage carry forward)
**Date:** 2026-05-27
...
> **Decision points 1 and 2 below are superseded by ADR-0033.** The session-key source is now the harness's transcript filename (production) with `AIT_MCP_TEST_SESSION_ID` as a test-only override; `CLAUDE_CODE_SESSION_ID` is no longer read. The encryption envelope still uses `sha256(uuid + ":ait-mcp:v2")` but the UUID comes from the resolver, not from that env var. Points 3, 4, and 5 carry forward unchanged.

1. ~~**Session key = `CLAUDE_CODE_SESSION_ID`.** Required, no fallback. If unset (non-Claude-Code runner), `loadIdentity` returns `null` and `saveIdentity` throws. Test runners must set it explicitly. ADR-0030's "no UUID parsing, no fallback" principle is preserved — one scheme, deterministic — only the source of the key changes.~~ *(Superseded by ADR-0033.)*

2. ~~**Encrypted credential envelope.** Sensitive fields (`password`, `accessJwt`, `refreshJwt`) live inside an AES-256-GCM ciphertext. Encryption key = `sha256(CLAUDE_CODE_SESSION_ID + ":ait-mcp:v2")`, derived only from the env var — never written to disk.~~ The plaintext outer envelope carries `did`, `handle`, `createdAt` (public protocol identifiers + a diagnostic timestamp). Fresh 12-byte nonce per write. *(Envelope structure carries forward; key source superseded by ADR-0033.)*
...
- ~~`CLAUDE_CODE_SESSION_ID` becomes a hard dependency of the MCP child.~~ *(Superseded by ADR-0033: the resolver now reads the harness's transcript filename in production; `AIT_MCP_TEST_SESSION_ID` is the test-only override for non-Claude-Code runners.)*
