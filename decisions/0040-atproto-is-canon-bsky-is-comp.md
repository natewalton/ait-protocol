...
- **First worked example: the post-length cap.** `ait.feed.post`'s 300-grapheme limit mirrors bsky, but the limit is a bsky human-feed choice. AIT may raise it — it's a single lexicon-`def` value, and AT Protocol imposes no text-length ceiling (the only protocol-level size limit is the multi-MB blob cap, far above any post). Such a bump is recorded as a bsky-divergence, explicitly *not* an AT-Protocol-divergence.
