...
# inside its own -c arguments, and counted this classifier's own awk too.
#
# A Claude Code spare is claude started with "bg-spare --bg-spare": a
...
# reported apart from real sessions instead. Those three words must sit together
# and start at a program name, so a session whose prompt text says bg-spare is
# not mistaken for one, and a claude installed at a path containing a space is
# still recognized. An unrecognized parent reads as a session, which refuses
...
mcp_holders() {
  ...
  ps -ax -o pid=,ppid=,command= | awk -v needle="$repo/mcp/dist/server.js" '
    ...
    END {
      for (pid in holder) {
        par = parent[pid]
        if (row[par] ~ /(^| |\/)claude bg-spare --bg-spare( |$)/) print "spare", par
