...
One behavior change too: **there is no forced bootstrap "join" turn.** A bare `codex-session.sh` runs no model turn — the operator joins by typing `join …` in the attached TUI, exactly like a bare `claude-session.sh` — while an opening prompt passed as an arg (`codex-session.sh "join AIT as @foo and wait"`) is injected as the first turn for a hands-off/autonomous session. **One codex-specific wrinkle:** a bare `thread/start` writes no on-disk *rollout*, and the TUI's `codex resume <threadId> --remote` needs one (it fails `no rollout found for thread id` otherwise — the old forced-join turn had incidentally created it). The driver therefore starts new threads with `historyMode: "legacy"` and calls `thread/name/set` (`"AIT codex session"`, the picker title), which writes an attachable `session_meta`-only rollout on codex-cli 0.144.4, 0.149.1, and 0.152 without injecting model-visible context or a synthetic turn. Before announcing the socket, it takes an `mcpServerStatus/list` snapshot and waits for terminal `mcpServer/startupStatus/updated` events for runtimes still starting, so a remote TUI cannot join partway through that notification round. The 12 short-lived paginated sessions created by the superseded inject seed are not migrated; start them fresh (see `specs/codex-resume-phantom-work.md`). No orientation is injected — the `ait` MCP's own `instructions` already cover how notifications arrive, and the operator controls how the session engages. Push registration depends on none of this — `registerPushWhenReady` polls for the identity file and registers once `join` happens.
...
3. **App-server client** (`mcp/src/codex/appServerClient.ts`) — a websocket-over-unix-socket JSON-RPC 2.0 client (Node `ws` via `ws+unix://PATH:/`): `initialize` + `initialized`, `thread/start`, `turn/start`, `turn/steer`; consumes `turn/started`/`turn/completed` to track the running turn's **id** (which `turn/steer` needs as its `expectedTurnId`); retries `-32001` with backoff. Types are generated (`codex app-server generate-ts`); regenerate on codex bumps.
...
5. **The sink** (`mcp/src/codex/sink.ts`) — `NotificationView → Codex turn`: format the turn text, choose `turn/steer` (a turn is running) vs `turn/start` (idle, carrying every pending notification at once), advance the cursor when the carrying turn completes.
...
4. Stream `item/started` / `item/completed` / deltas (or opt out), then `turn/completed { threadId, turn }` (final state + token usage). `turn/interrupt` cancels; `turn/steer { threadId, input, expectedTurnId }` injects into the running turn (needs the active turn id from `turn/started`); the sink uses it for every notification that arrives mid-turn. `turn/interrupt` stays reserved.
5. **`thread/inject_items { threadId, items }`** appends raw items to model-visible history **without** starting a turn (silent context, no response) — noted as the "attach context, don't provoke" alternative, not our promotion or rollout-seeding path. A Codex 0.152 production-shaped probe reproduced the same dangling `auto-compact-0` context and phantom-`Working` failure previously seen on 0.144.4, so the driver must not use this operation to make a new thread attachable.
...
| :--- | :--- | :--- |
| `thread/inject_items` | appends to model-visible history, **no turn triggered**; also the rollout seed for new threads on 0.152 (see behavior-change note) | *weaker* — adds context but doesn't provoke a response |
...
- **Turn in flight** → **`turn/steer`** into it, so the model reads the notification during that turn. This replaced the original enqueue-until-the-boundary rule, which made arrival time depend on the length of whatever was already running: a measured 84-minute turn delivered its last queued notification 89 minutes late. Steering is deliberately *more* aggressive than a Claude channel, which never reaches the model mid-turn.
- **A turn that refuses steering** (`review`, `compact` — the protocol's `NonSteerableTurnKind`) refuses `turn/start` for the same reason, so the batch waits for that turn to end rather than failing twice.
- **Ordering and loss** are unchanged: delivery stays ordered and nothing is dropped. A `-32001` (server overloaded) is retried with backoff. The cursor still advances only when the carrying turn completes successfully.
- **`turn/interrupt`** remains unused — cancelling an operator's turn for a notification is a different decision from adding to it.
...
- **`turn/interrupt` policy.** `turn/steer` now carries every notification into a running turn (see Delivery semantics). Cancelling a turn outright for an urgent notification is still undesigned: which notifications would qualify, and what the model does with work abandoned mid-flight.
